...
const getStatusAll = () => {
  return axiosInstance.get("/reservation/status");
};
...
const get = (rid: number) => {
  return axiosInstance.get(`/reservation/${rid}`);
};
...
const update = (rid: number, data: IReservation) => {
  return axiosInstance.put(`/reservation/${rid}`, data);
};

// 삭제함수
const remove = (rid: number) => {
  return axiosInstance.delete(`/reservation/${rid}`);
};
