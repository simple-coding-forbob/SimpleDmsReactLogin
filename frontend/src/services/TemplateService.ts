...
const viewPdf = (docId: number) => {
  return axiosInstance.get(`/template/pdf/${docId}`, {
...
const get = (docId: number) => {
  return axiosInstance.get(`/template/${docId}`);
};

// 삭제
const remove = (docId: number) => {
  return axiosInstance.delete(`/template/${docId}`);
};
...
const insert = (data: ITemplate) => {
  ...
  return axiosInstance.post("/template", formData, {
